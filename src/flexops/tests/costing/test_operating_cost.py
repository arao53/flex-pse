...
_DAY_SCALE = 24.0 / 744.0  # one day of a 31-day month


def _saturated_prev_demand_dict(tariff, index):
    """A prior-demand carry above any peak this horizon, so EECO bills 0 demand."""
    charge_dict = opex._charge_dict(tariff, index, 1.0)
    return {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            False,
            _ONE_DAY_ENERGY_AT_100KW + _MONTHLY_ASSESSED_AT_100KW,
        ),
        # prorate on: demand + customer scaled to the day's fraction of the month
        (
            True,
            False,
            False,
...
def test_evaluate_cost_sub_billing_period(
    ...
    index, load = (
        (_july_index(), _reference_load())
        if full_month
        else (_one_day_index(), np.full(24, 100.0))
    )
    prev = _saturated_prev_demand_dict(tariff, index) if carry_demand else None
    total = evaluate_cost(
        load,
        tariff,
        dt_hours=1.0,
