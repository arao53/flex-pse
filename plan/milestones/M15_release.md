...
- `plan/02_testing_and_ci.md` §3 (PR-CI per-subpackage coverage reporting; upstream-canary workflow), §4 (coverage policy: "M15 turns on gentle floors")
...
### 2. Upstream canary + hardening

- Manually dispatch `.github/workflows/upstream-canary.yml` (`gh workflow run
  upstream-canary.yml` or the Actions UI) and wait for the result.
- Green: note the run URL in the PR. Red: for each failure either **fix** it in
  `flexcore/compat/` (the designated churn point, architecture §2.1) or **pin**
  the offending upstream version range in `flexcore/compat/versions.py` +
  `pyproject.toml` and **file a GitHub issue** describing the breakage. No
  release with an unexplained red canary.
- Run the full local gauntlet once: `pytest -q` (all tiers, needs solvers)
  and both docs build modes (03 §6).
...
the existing ci.yml jobs with the least machinery. Keep the exclusions of
02 §4 (`flexops/testing/`, `flexcore/compat/`).
...
the hosted docs; license (BSD-3-Clause). Keep it under a screen and a half.
...
7. **A red canary "noted for later".** Fix or pin+issue before tagging —
   releasing on a known-broken upstream combination without a pin ships
   breakage to every new install.
...
- [ ] Upstream canary run green, or every red item fixed or pinned with a filed issue linked in the PR
