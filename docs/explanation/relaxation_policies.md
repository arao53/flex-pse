...
MINLP directly with a MINLP-capable solver is not the silent transformation
this facade refuses to do; it is the sanctioned path ("install a
MINLP-capable solver"). What the facade will never do on its own is relax
integrality to make a MINLP fit a MILP solver.
...
When a MINLP-capable solver *is* available it is used directly. For cases
where a direct MINLP solve is intractable, an explicit relax → solve-MIP →
fix → solve-NLP pipeline is planned as part of the rolling-horizon scheduling
engine, which does not exist yet. The `ProblemClass.MINLP_OA` (outer
approximation) and `ProblemClass.MINLP_TR` (trust region) enum members are
reserved strategy slots for that pipeline, documented but deliberately
unimplemented today; `classify` never returns them.
